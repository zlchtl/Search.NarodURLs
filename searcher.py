...
def create_urls(n = 4, chunk_size = 2000, start_from=None):
    '''Функция генерации массива ссылок по комбинациям и с последующим разбиением данных на чанки'''

    alphabet = (chr(i) for i in range(97, 123))
    data = ['https://'+''.join(combination)+'.narod.ru/' for combination in product(alphabet, repeat=n)]
    start_index = 0
    if start_from:
        try:
